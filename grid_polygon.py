...
y_spacing = x_spacing
dot_radius = 0.015625 * pxPerInch # 0.015625 = 1/64th
...
save_file_name = "grid_poly_" + str(number_verts) + "_sides_" + str(intervals_per_inch) + "_per_inch"
save_file_full_path = save_file_directory + "/" + save_file_name

#trace_style = "stroke: rgb(200,200,200); stroke-width:0.; fill: none;"
dot_style = "stroke: none; fill: rgb(200,200,200);"
...
for row in range(0,y_count):
    for col in range(0, x_count):
        ...
        x_cord = offsetX+(col*(x_spacing)) #+ (row%2 * x_spacing/2)
        polygon(x_cord, y_cord, dot_radius, number_verts)
